...
ZPLC (Zephyr PLC) is a portable IEC 61131-3 runtime and development environment
for modern industrial automation. Timing, determinism, board support, and safety
claims are evidence-gated; source presence alone is not a product qualification.
...
4.  **Documentation Standards**: When updating public documentation, follow the structure established in `docs/docs/`. Ensure that both English and Spanish (`docs/i18n/es/`) versions are updated in parallel to maintain parity and distinguish host, QEMU, target, HIL, and manual evidence.
...
7.  **Physical Operations**: Keep build, flash, deploy, and run/debug separate. AI and MCP must not initiate flash, deploy, force, RUN/STOP, raw serial, or shell actions.
...
*   `.specify/`: Contains the repository engineering constitution.
*   `specs/`: Contains feature specifications, release evidence, and approved RFCs.
...
*   **PLC programming and agent work**: Load `.agents/skills/zplc-plc-programmer/SKILL.md` before inspecting, editing, compiling, or testing ZPLC PLC projects.
