...
export interface DirectoryIdentity { path: string; dev: number; ino: number; }
...
export async function directoryIdentity(path: string): Promise<DirectoryIdentity | undefined> {
  try { const initial = await lstat(path); if (initial.isSymbolicLink() || !initial.isDirectory()) return undefined; const canonical = await realpath(path); const status = await lstat(canonical); return !status.isSymbolicLink() && status.isDirectory() ? { path: canonical, dev: status.dev, ino: status.ino } : undefined; } catch { return undefined; }
}
async function sameDirectory(identity: DirectoryIdentity): Promise<boolean> { const current = await directoryIdentity(identity.path); return !!current && current.path === identity.path && current.dev === identity.dev && current.ino === identity.ino; }
...
export async function runFirmwareBuild(repositoryRoot: string, zephyrBoard: string, options: FirmwareBuildRunnerOptions = {}): Promise<FirmwareBuildRunResult> {
  ...
  try {
    result = await new Promise<FirmwareBuildRunResult>((finish) => {
      ...
      const child = (options.spawnProcess ?? spawn)('west', ['build', '-b', zephyrBoard, 'firmware/app', '-d', owned.path, '--pristine'], { cwd: root.path, shell: false, stdio: ['ignore', 'pipe', 'pipe'], windowsHide: true, detached: true });
      ...
      const settle = async (code?: number | null) => {
        ...
        const artifact = await readArtifact(owned.path); finish(artifact ? { ok: true, artifact, output: observed } : { ok: false, failure: 'artifact-invalid', output: observed });
      };
