...
async function main(): Promise<void> {
  ...
  try {
    ...
    const copiedExample = await waitFor('copied Motor Start Stop example', async () => await cdp!.evaluate(`import('/src/store/useIDEStore.ts').then(({ useIDEStore }) => { const state = useIDEStore.getState(); const copy = window.__zplcSmokeExampleCopy; return state.isProjectOpen && state.directoryHandle === copy.handle ? { pickerCalls: copy.pickerCalls, isVirtualProject: state.isVirtualProject, migrationPreview: state.projectMigrationPreview, scenario: copy.files.get('tests/motor-start-stop.scenario.json'), active: state.activeFileId } : undefined; })`) as { pickerCalls: number; isVirtualProject: boolean; migrationPreview: unknown; scenario: string | undefined; active: string | null } | undefined);
    assert(copiedExample?.pickerCalls === 1 && copiedExample.isVirtualProject === false && copiedExample.migrationPreview === null && typeof copiedExample.scenario === 'string' && copiedExample.active === 'file:src/Motor.st', `Example copy did not finish as one disk-backed session: ${JSON.stringify(copiedExample)}`);
