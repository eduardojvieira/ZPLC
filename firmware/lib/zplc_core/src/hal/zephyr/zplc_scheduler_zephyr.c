...
static void sync_inputs_to_ipi(void) {
  ...
  for (int ch = 0; ch < ZPLC_ADC_CHANNEL_COUNT; ch++) {
    if (zplc_hal_adc_read(ch, &adc_val) == ZPLC_HAL_OK) {
...
static void task_timer_handler(struct k_timer *timer) {
  ...
  /* Submit work */
  t->work_pending = 1;
  k_work_submit_to_queue(get_workq_for_priority(t->task.config.priority),
                         &t->work);
}
...
zplc_sched_state_t zplc_sched_get_state(void) { return sched_state; }
